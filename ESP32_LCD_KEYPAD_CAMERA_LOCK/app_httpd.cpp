...
#define STREAM_FPS_LOG_EVERY_SEC 1
#define STREAM_ADVERTISED_FPS "30"
...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  httpd_resp_set_hdr(req, "X-Framerate", STREAM_ADVERTISED_FPS);
  ...
  while (true) {
    ...
    if (res != ESP_OK) {
      log_e("[STREAM] send failed res=0x%x", (unsigned)res);
      break;
    }
    ...
    if (now - t_start >= 1000000) {
      float fps = frames * 1000000.0f / (now - t_start);
      log_i("[STREAM] FPS=%.1f  last=%uB", fps, (unsigned)_jpg_buf_len);
      frames = 0;
      t_start = now;
    }
#endif
...
static esp_err_t cmd_handler(httpd_req_t *req) {
  ...
  if (!strcmp(variable, "framesize")) {
    if (s->pixformat == PIXFORMAT_JPEG) r = s->set_framesize(s, (framesize_t)val);
  } else if (!strcmp(variable, "quality")) {
    r = s->set_quality(s, val);
  } else if (!strcmp(variable, "contrast")) {
    r = s->set_contrast(s, val);
  } else if (!strcmp(variable, "brightness")) {
    r = s->set_brightness(s, val);
  } else if (!strcmp(variable, "saturation")) {
    r = s->set_saturation(s, val);
  } else if (!strcmp(variable, "gainceiling")) {
    r = s->set_gainceiling(s, (gainceiling_t)val);
  } else if (!strcmp(variable, "colorbar")) {
    r = s->set_colorbar(s, val);
  } else if (!strcmp(variable, "awb")) {
    r = s->set_whitebal(s, val);
  } else if (!strcmp(variable, "agc")) {
    r = s->set_gain_ctrl(s, val);
  } else if (!strcmp(variable, "aec")) {
    r = s->set_exposure_ctrl(s, val);
  } else if (!strcmp(variable, "hmirror")) {
    r = s->set_hmirror(s, val);
  } else if (!strcmp(variable, "vflip")) {
    r = s->set_vflip(s, val);
  } else if (!strcmp(variable, "awb_gain")) {
    r = s->set_awb_gain(s, val);
  } else if (!strcmp(variable, "agc_gain")) {
    r = s->set_agc_gain(s, val);
  } else if (!strcmp(variable, "aec_value")) {
    r = s->set_aec_value(s, val);
    ...
    r = s->set_raw_gma(s, val);
  } else if (!strcmp(variable, "lenc")) {
    r = s->set_lenc(s, val);
  } else if (!strcmp(variable, "special_effect")) {
    r = s->set_special_effect(s, val);
  } else if (!strcmp(variable, "wb_mode")) {
    r = s->set_wb_mode(s, val);
  } else if (!strcmp(variable, "ae_level")) {
    r = s->set_ae_level(s, val);
  }
#if defined(LED_GPIO_NUM)
...
static esp_err_t xclk_handler(httpd_req_t *req) {
  ...
  int xclk = atoi(_xclk);
  sensor_t *s = esp_camera_sensor_get();
  int r = s->set_xclk(s, LEDC_TIMER_0, xclk);
  if (r) return httpd_resp_send_500(req);
...
static esp_err_t greg_handler(httpd_req_t *req) {
  ...
  sensor_t *s = esp_camera_sensor_get();
  int r = s->get_reg(s, reg, mask);
  if (r < 0) return httpd_resp_send_500(req);

  char buffer[20];
  const char *val = itoa(r, buffer, 10);
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
...
static esp_err_t pll_handler(httpd_req_t *req) {
  ...
  sensor_t *s = esp_camera_sensor_get();
  int r = s->set_pll(s, bypass, mul, sys, root, pre, seld5, pclken, pclk);
  if (r) return httpd_resp_send_500(req);
...
static esp_err_t win_handler(httpd_req_t *req) {
  ...
  sensor_t *s = esp_camera_sensor_get();
  int r = s->set_res_raw(s, startX, startY, endX, endY, offsetX, offsetY, totalX, totalY, outputX, outputY, scale, binning);
  if (r) return httpd_resp_send_500(req);
